...
modes_de_vente = ['A emporter', 'Sur place']
...
while current_date <= end_date:
    for nom, adresse in restaurants:
        ...
        mode_de_vente = random.choice(modes_de_vente)
        
        data.append([nom, adresse, current_date.strftime('%Y-%m-%d'), chiffre_affaire, nb_commandes, mode_de_vente, panier_moyen])
    current_date += timedelta(days=1)

columns = ['Nom', 'Adresse', 'Date', 'ChiffreAffaire', 'NbCommandes', 'Mode de Vente', 'Panier Moyen']
df = pd.DataFrame(data, columns=columns)
df.to_csv('mode_de_vente_data.csv', index=False)
