...
while current_date <= end_date:
    for nom, adresse in restaurants:
        ...
        data.append([famille ,nom, adresse, current_date.strftime('%Y-%m-%d'), nb_commandes, chiffre_affaire, panier_moyen])
    current_date += timedelta(days=1)

columns = ['famille', 'Nom', 'Adresse', 'Date', 'NbCommandes', 'Chiffre Affaire', 'Panier Moyen']
df = pd.DataFrame(data, columns=columns)
df.to_csv('Familles_produits_data.csv', index=False)
