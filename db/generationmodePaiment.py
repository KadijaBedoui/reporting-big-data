...
modes_de_paiement = ['Carte de crédit', 'Espèces', 'Chèque', 'PayPal', 'Apple Pay', 'Google Pay']
...
while current_date <= end_date:
    for nom, adresse in restaurants:
        ...
        mode_de_paiement = random.choice(modes_de_paiement)
        
        data.append([nom, adresse, current_date.strftime('%Y-%m-%d'), chiffre_affaire, nombre_commandes, panier_moyen, mode_de_paiement])
    current_date += timedelta(days=1)

columns = ['Nom', 'Adresse', 'Date', 'Chiffre Affaire', 'Nombre Commandes', 'Panier Moyen', 'Mode de Paiement']
df = pd.DataFrame(data, columns=columns)

df.to_csv('mode_de_paiement_data.csv', index=False)
